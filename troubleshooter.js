...
`,options:[{id:"serialization",page:342,label:"Serialization",summary:"The issue concerns serialization"},{id:"deserialization",page:283,label:"Deserialization",summary:"The issue concerns deserialization"}]},{content:`<p>What is the value of <code>DeserializationError</code>?</p>
`,options:[{id:"ok",page:307,label:"<code>Ok</code>",summary:"`deserializeJson()` returns `Ok`"},{id:"emptyinput",page:296,label:"<code>EmptyInput</code>",summary:"`deserializeJson()` returns `EmptyInput`"},{id:"incompleteinput",page:300,label:"<code>IncompleteInput</code>",summary:"`deserializeJson()` returns `IncompleteInput`"},{id:"invalidinput",page:304,label:"<code>InvalidInput</code>",summary:"`deserializeJson()` returns `InvalidInput`"},{id:"nomemory",page:330,label:"<code>NoMemory</code>",summary:"`deserializeJson()` returns `NoMemory`"},{id:"toodeep",page:216,label:"<code>TooDeep</code>",summary:"`deserializeJson()` returns `TooDeep`"},{id:"crash",page:285,label:"I can't tell because the program crashes",summary:"The program crashes"},{id:"unknown",page:284,label:"I don't know what you're talking about",summary:"The program doesn't check the error"}]},{content:`<p><code>DeserializationError</code> is the return type of <code>deserializeJson()</code>. It tells whether the operation succeeded and indicates the cause of the error.</p>
<p>Modify your program to show the error code, like so:</p>
<pre><code class="hljs language-c++">DeserializationError error = <span class="hljs-built_in">deserializeJson</span>(doc, input);
...
<p>How many traces can you see in the Serial Monitor?</p>
`,options:[{id:"before",page:294,label:"None: program crashes before calling <code>deserializeJson()</code>",summary:"Program crashes before calling `deserializeJson()`"},{id:"inside",page:295,label:"Only one: program crashes inside <code>deserializeJson()</code>",summary:"Program crashes inside `deserializeJson()`"},{id:"after",page:286,label:"Two traces: program crashes after calling <code>deserializeJson()</code>",summary:"Program crashes after calling `deserializeJson()`"}]},{content:`<p>So, the program crashes when you try to extract a value from the JSON document.</p>
<p>What is the type of the variable you're extracting from?</p>
`,options:[{id:"jsonarray",page:289,label:"<code>JsonArray</code>",summary:"The program crashes when reading a value from a `JsonArray`"},{id:"jsondocument",page:287,label:"<code>JsonDocument</code>",summary:"The program crashes when reading a value from the `JsonDocument`"},{id:"jsonobject",page:291,label:"<code>JsonObject</code>",summary:"The program crashes when reading a value from a `JsonObject`"},{id:"jsonvariant",page:293,label:"<code>JsonVariant</code>",summary:"The program crashes when reading a value from a `JsonVariant`"}]},{content:`<p>Programs often crash because they keep a pointer to a string stored in the <code>JsonDocument</code>, even after the <code>JsonDocument</code> has been destructed.</p>
<p>Indeed, when <code>JsonDocument</code> returns a <code>const char*</code>, it doesn't return a copy of the string, but the address of the string in the memory pool. When the <code>JsonDocument</code> is destructed, the memory pool gets released and the pointer dangles. Later, when the program tries to use the string, it reads at an invalid memory location and crashes.</p>
<p>For example, it happens with the following program:</p>
<pre><code class="hljs language-c++"><span class="hljs-type">const</span> <span class="hljs-type">char</span>* username;
...
`,options:[{id:"dangling",page:288,label:"Yes",summary:"The `JsonArray` outlives the `JsonDocument`"},{id:"not-dangling",page:287,label:"No",summary:"The `JsonArray` doesn't outlive the `JsonDocument`"}]},{content:`<p>You must refactor your program so that it doesn't keep the <code>JsonObject</code> alive longer than the <code>JsonDocument</code>.</p>
...
`,options:[{id:"dangling",page:292,label:"Yes",summary:"The `JsonVariant` outlives the `JsonDocument`"},{id:"not-dangling",page:287,label:"No",summary:"The `JsonVariant` doesn't outlive the `JsonDocument`"}]},{content:`<p>I think this could be a stack-overflow.</p>
<p>Please check in your code if there aren't some large variable in the stack. Look for things like:</p>
<ul>
<li><code>char buffer[1024]</code></li>
<li><code>int matrix[64][64][64]</code></li>
...
<p>Please review your code to make sure you're not passsing a dangling pointer to <code>deserializeJson()</code>.</p>
<p>Does your program still crash?</p>
`,options:[{id:"no-dangling-ptr",page:294,label:"Yes",summary:"Removing dangling pointers doesn't prevent the crash"},{id:"success",page:2,label:"No",summary:"Removing dangling pointers prevents the crash"}]},{content:`<p><code>EmptyInput</code> means that the input was empty or contained only spaces or comments.</p>
<p>Where does your JSON input come from?</p>
`,options:[{id:"http",page:109,label:"An HTTP response",summary:"Input comes from an HTTP response"},{id:"file",page:297,label:"A file",summary:"Input comes from a file"},{id:"serial",page:299,label:"A serial port",summary:"Input comes from a serial port"},{id:"other",page:116,label:"Something else",summary:"Input comes neither from an HTTP response, nor a file, nor a serial port"}]},{content:`<p><code>EmptyInput</code> in the context of a file usually means:</p>
<ul>
<li>the file is closed</li>
<li>the file is opened in the wrong mode</li>
<li>the file is empty</li>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Adding a wait loop fixes the issue"},{id:"available",page:298,label:"No",summary:"Adding a wait loop doesn't fix the issue"}]},{content:`<p><code>IncompleteInput</code> means that <code>deserializeJson()</code> managed to parse the beginning of the JSON document, but the end was missing.</p>
<p>What type of input do you pass to <code>deserializeJson()</code>?</p>
`,options:[{id:"stream",page:122,label:"A stream",summary:"Input comes from a stream"},{id:"string",page:301,label:"A string object (like <code>String</code>)",summary:"Input comes from a string object"},{id:"pointer",page:302,label:"A pointer (like <code>char*</code> or <code>const char*</code>)",summary:"Input comes from a pointer"}]},{content:`<p>Please print the content of the string to the serial port, like so:</p>
<pre><code class="hljs language-c++">Serial.<span class="hljs-built_in">println</span>(input);
</code></pre>
<p>You should see that the JSON document is truncated.
This can be caused by a lack of RAM: the string object failed to allocate a buffer large enough for the whole document.</p>
...
<p>Hopefully, you'll be able to reserve a buffer large enough to store the whole string.</p>
<p>Did this solve your issue?</p>
`,options:[{id:"0",page:2,label:"Yes",summary:"Calling `String::reserve()` solves the issue"},{id:"1",page:303,label:"No",summary:"Calling `String::reserve()` doesn't solve the issue"}]},{content:`<p>I'll assume that your input is stored in a buffer similar to this one:</p>
<pre><code class="hljs language-c++"><span class="hljs-type">char</span> input[<span class="hljs-number">1024</span>];
<span class="hljs-type">size_t</span> input_size;
</code></pre>
<p>Please print the content of the string to the serial port, like so:</p>
...
<p>Please increase the capacity of the buffer, but remember that the size of the stack is limited so you may need to move your buffer to the heap.</p>
<p>Did this solve your issue?</p>
`,options:[{id:"0",page:2,label:"Yes",summary:"Increasing the buffer size solves the issue"},{id:"1",page:303,label:"No",summary:"Increasing the buffer size doesn't solve the issue"}]},{content:`<p>To reduce the memory usage, you should try to pass the source (file, connection, stream, etc) directly to <code>deserializeJson()</code>, either via the <a href="https://www.arduino.cc/reference/en/language/functions/communication/stream/"><code>Stream</code></a> interface or via a <a href="/news/2019/11/01/version-6-13-0/">custom reader</a>. This will avoid the need for an intermediate buffer.</p>
<p>Additionaly, you can add a <a href="/news/2020/03/22/version-6-15-0/">filter</a> to remove all unneeded information from the <code>JsonDocument</code>.</p>
<p>You can see examples here:</p>
<ul>
...
`,options:[{id:"software",page:160,label:"Yes",summary:"Program uses `SoftwareSerial`"},{id:"hardware",page:305,label:"No",summary:"Program doesn't use `SoftwareSerial`"}]},{content:`<p>So, what's the problem then?</p>
`,options:[{id:"slow",page:194,label:"It's slow",summary:"Deserialisation is slow"},{id:"invalid",page:169,label:"I expected <code>InvalidInput</code> instead of <code>Ok</code>",summary:"`Ok` is incorrect; it should be `InvalidInput`"},{id:"crash",page:286,label:"My program crashes",summary:"Program crashes"},{id:"empty",page:308,label:"<code>JsonDocument</code> returns empty/null values",summary:"Program fails to extract values from the `JsonDocument`"},{id:"truncated-strings",page:195,label:"Some strings are truncated",summary:"Some strings are truncated"}]},{content:`<p>Do you use a filter (with <code>DeserializationOption::Filter</code>)?</p>
`,options:[{id:"filter",page:310,label:"Yes",summary:"Program uses `DeserializationOption::Filter`"},{id:"no-filter",page:315,label:"No",summary:"Program doesn't use `DeserializationOption::Filter`"}]},{content:`<p>When a program fails to extract the values from a <code>JsonDocument</code>, it's often because of confusion between arrays and objects.</p>
<p>For example, a common mistake is to write:</p>
<pre><code class="hljs language-c++"><span class="hljs-type">const</span> <span class="hljs-type">char</span>* temperature = doc[<span class="hljs-string">&quot;list&quot;</span>][<span class="hljs-string">&quot;main&quot;</span>][<span class="hljs-string">&quot;temp&quot;</span>];
</code></pre>
<p>instead of</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"The issue is confusion between array and object"},{id:"array-object",page:319,label:"No",summary:"The issue is not confusion between array and object"}]},{content:`<p>Please print the value of <code>JsonDocument::overflowed()</code>, like so:</p>
...
<p>What value does it print?</p>
`,options:[{id:"overflowed-1",page:312,label:"<code>1</code> (or <code>true</code>)",summary:"`filter.overflowed()` returns `true`"},{id:"overflowed-0",page:311,label:"<code>0</code> (or <code>false</code>)",summary:"`filter.overflowed()` returns `false`"}]},{content:`<p>Maybe the filter excludes the part that it should keep. Let's verify that it's doing its job correctly.</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Freeing some memory solves the issue."},{id:"failure",page:311,label:"No",summary:"Freeing some memory doesn't solve the issue."}]},{content:`<p>Which function do you call to deserialize the input?</p>
`,options:[{id:"json",page:1,label:"<code>deserializeJson()</code>",summary:"Program calls `deserializeJson()`"},{id:"msgpack",page:314,label:"<code>deserializeMsgPack()</code>",summary:"Program calls `deserializeMsgPack()`"}]},{content:`<p>It seems that your program fed <code>deserializeMsgPack()</code> with garbage input.</p>
<p>Indeed, any byte in the following ranges is a valid one-byte MessagePack document:</p>
<ul>
<li><code>0x00</code>-<code>0x7f</code>: positive fixint</li>
<li><code>0xc0</code>: nil</li>
...
<p>What's the first character?</p>
`,options:[{id:"quote",page:318,label:"A quote (<code>&quot;</code>)",summary:"Serialized document starts with a quote"},{id:"bracket",page:309,label:"A square bracket (<code>[</code>)",summary:"Serialized document starts with a square bracket"},{id:"brace",page:309,label:"A curly brace (<code>{</code>)",summary:"Serialized document starts with a curly brace"},{id:"not-in-list",page:313,label:"None of the above",summary:"Serialized document starts with neither a quote, a bracket, nor a brace."}]},{content:`<p>If you cannot fix the server, you can at least reverse the double serialization (stringification) to get back the original object.</p>
<p>To do this, you must call <code>deserializeJson()</code> twice, like so:</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Fixing the server fixes the issue"},{id:"server-ok",page:316,label:"No",summary:"Fixing the server doesn't fix the issue"}]},{content:`<p>If you see a double-quote at the beginning of the JSON input, it means that the whole document is neither an object or an array; it's a string. This happens when the input was incorrectly generated, by serializing (stringifying) to JSON twice.</p>
<p>For example, the following document:</p>
<pre><code class="hljs language-json"><span class="hljs-string">&quot;{\\&quot;hello\\&quot;:\\&quot;world\\&quot;}&quot;</span>
</code></pre>
<p>...is not a JSON <em>object</em> but a <em>string</em>.
...
<p>So, the bug isn't in the Arduino code, but on the other size, most likely a server.</p>
<p>Can you modify the code of the server?</p>
`,options:[{id:"server-fixable",page:317,label:"Yes",summary:"User can modify the server"},{id:"server-unfixable",page:316,label:"No",summary:"User cannot modify the server"}]},{content:`<p>What type of value are you trying to read?</p>
`,options:[{id:"bool",page:1,label:"<code>bool</code>",summary:"The program fails to extract a `bool` value"},{id:"const-char-ptr",page:1,label:"<code>const char*</code>",summary:"The program fails to extract a `const char*` value"},{id:"double",page:1,label:"<code>double</code>",summary:"The program fails to extract a `double` value"},{id:"float",page:1,label:"<code>float</code>",summary:"The program fails to extract a `float` value"},{id:"int",page:1,label:"<code>int</code>",summary:"The program fails to extract an `int` value"},{id:"int8",page:324,label:"<code>int8_t</code>",summary:"The program fails to extract an `int8_t` value"},{id:"int16",page:322,label:"<code>int16_t</code>",summary:"The program fails to extract an `int16_t` value"},{id:"int32",page:323,label:"<code>int32_t</code>",summary:"The program fails to extract an `int32_t` value"},{id:"int64",page:1,label:"<code>int64_t</code>",summary:"The program fails to extract an `int64_t` value"},{id:"long",page:323,label:"<code>long</code>",summary:"The program fails to extract a `long` value"},{id:"long-long",page:1,label:"<code>long long</code>",summary:"The program fails to extract a `long long` value"},{id:"short",page:322,label:"<code>short</code>",summary:"The program fails to extract a `short` value"},{id:"signed-char",page:324,label:"<code>signed char</code>",summary:"The program fails to extract a `signed char` value"},{id:"signed-int",page:322,label:"<code>signed int</code>",summary:"The program fails to extract a `signed int` value"},{id:"signed-long",page:323,label:"<code>signed long</code>",summary:"The program fails to extract a `signed long` value"},{id:"signed-long-long",page:1,label:"<code>signed long long</code>",summary:"The program fails to extract a `signed long long` value"},{id:"signed-short",page:322,label:"<code>signed short</code>",summary:"The program fails to extract a `signed short` value"},{id:"std-string",page:1,label:"<code>std::string</code>",summary:"The program fails to extract a `std::string` value"},{id:"uint8",page:329,label:"<code>uint8_t</code>",summary:"The program fails to extract a `uint8_t` value"},{id:"uint16",page:327,label:"<code>uint16_t</code>",summary:"The program fails to extract a `uint16_t` value"},{id:"uint32",page:327,label:"<code>uint32_t</code>",summary:"The program fails to extract a `uint32_t` value"},{id:"uint64",page:1,label:"<code>uint64_t</code>",summary:"The program fails to extract a `uint64_t` value"},{id:"unsigned-char",page:329,label:"<code>unsigned char</code>",summary:"The program fails to extract an `unsigned char` value"},{id:"unsigned-int",page:328,label:"<code>unsigned int</code>",summary:"The program fails to extract an `unsigned int` value"},{id:"unsigned-long",page:328,label:"<code>unsigned long</code>",summary:"The program fails to extract an `unsigned long` value"},{id:"unsigned-long-long",page:1,label:"<code>unsigned long long</code>",summary:"The program fails to extract an `unsigned long long` value"},{id:"unsigned-short",page:327,label:"<code>unsigned short</code>",summary:"The program fails to extract an `unsigned short` value"},{id:"array",page:321,label:"<code>JsonArray</code>",summary:"The program fails to extract a `JsonArray`"},{id:"object",page:326,label:"<code>JsonObject</code>",summary:"The program fails to extract a `JsonObject`"},{id:"other",page:1,label:"None of the above",summary:"The type of the extracted value is not listed"}]},{content:`<p><code>to&lt;JsonArray&gt;()</code> replaces the existing value with a new empty array.</p>
...
`,options:[{id:"implicit",page:1,label:"<code>JsonObject object = doc[&quot;key&quot;];</code>",summary:"The program uses implicit conversion to `JsonObject`"},{id:"as",page:1,label:"<code>doc[&quot;key&quot;].as&lt;JsonObject&gt;()</code>",summary:"The program uses `as<JsonObject>()`"},{id:"to",page:325,label:"<code>doc[&quot;key&quot;].to&lt;JsonObject&gt;()</code>",summary:"The program uses `to<JsonObject>()`"}]},{content:`<p>This type can only store values between 0 and 65,535.<br>
If you try to extract a value outside this range, ArduinoJson will return 0.<br>
If that's your case, you must use a larger integer type.</p>
<p>Did this solve your issue?</p>
...
`,options:[{id:"jsonarray",page:331,label:"<code>JsonArray</code>",summary:"The first argument of `deserializeJson()` is a `JsonArray`"},{id:"jsondocument",page:334,label:"<code>JsonDocument</code>",summary:"The first argument of `deserializeJson()` is a `JsonDocument`"},{id:"jsonobject",page:332,label:"<code>JsonObject</code>",summary:"The first argument of `deserializeJson()` is a `JsonObject`"},{id:"jsonvariant",page:333,label:"<code>JsonVariant</code>",summary:"The first argument of `deserializeJson()` is a `JsonVariant`"}]},{content:`<p>You can pass a <code>JsonArray</code> as the first argument of <code>deserializeJson()</code>, but it has to point to an existing array.</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Initializing the array fixes the issue"},{id:"server-ok",page:334,label:"No",summary:"Initializing the array doesn't fix the issue"}]},{content:`<p>You can pass a <code>JsonObject</code> as the first argument of <code>deserializeJson()</code>, but it has to point to an existing object.</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Initializing the variant fixes the issue"},{id:"server-ok",page:334,label:"No",summary:"Initializing the variant doesn't fix the issue"}]},{content:`<p>Let's check with the <a href="/v7/assistant/">ArduinoJson Assistant</a> to see if you have enough memory for your JSON document.</p>
<ol>
...
`,options:[{id:"loose",page:335,label:"There is a lot more memory than required",summary:"The Assistant says there is a lot more memory than required."},{id:"tight",page:336,label:"There should be enough memory",summary:"The Assistant says there should be just enough memory."},{id:"too-small",page:341,label:"There is not enough memory",summary:"The Assistant says there is not enough memory."}]},{content:`<p>What CPU architecture is this code running on?</p>
`,options:[{id:"8-bit",page:337,label:"8-bit",summary:"The code is running on an 8-bit CPU"},{id:"32-bit",page:338,label:"32-bit",summary:"The code is running on a 32-bit CPU"},{id:"64-bit",page:340,label:"64-bit",summary:"The code is running on a 64-bit CPU"}]},{content:`<p>This means that ArduinoJson couldn't allocate enough memory for the JSON document.<br>
So you must free some memory.</p>
<p>See <a href="/v7/how-to/reduce-memory-usage/">How to reduce memory usage?</a> for some tips.</p>
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Freeing some memory solves the issue."},{id:"failure",page:341,label:"No",summary:"Freeing some memory doesn't solve the issue."}]},{content:`<p>Due to an optimization, a <code>JsonDocument</code> can only contain up to 255 nodes on an 8-bit CPU.
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Setting `ARDUINOJSON_SLOT_ID_SIZE` to 2 solves the issue."},{id:"slot-id-size-2",page:339,label:"No",summary:"Setting `ARDUINOJSON_SLOT_ID_SIZE` to 2 doesn't solve the issue."},{id:"fewer-than-255",page:339,label:"My JSON document contains fewer than 255 nodes",summary:"The JSON document contains fewer than 255 nodes."}]},{content:`<p>Due to an optimization, a <code>JsonDocument</code> can only contain up to 65,535 nodes on a 32-bit CPU.
A node is a value, a key, or a member of an array.</p>
<p>You can push this limit by increasing <code>ARDUINOJSON_SLOT_ID_SIZE</code>, like so:</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Setting `ARDUINOJSON_SLOT_ID_SIZE` to 4 solves the issue."},{id:"slot-id-size-2",page:340,label:"No",summary:"Setting `ARDUINOJSON_SLOT_ID_SIZE` to 4 doesn't solve the issue."},{id:"fewer-than-255",page:340,label:"My JSON document contains fewer than 65,535 nodes",summary:"The JSON document contains fewer than 65,535 nodes."}]},{content:`<p>Due to another optimization, strings in a <code>JsonDocument</code> can only contain up to 255 characters on an 8-bit CPU.</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Setting `ARDUINOJSON_STRING_LENGTH_SIZE` to 2 solves the issue"},{id:"string-length-size-2",page:336,label:"No",summary:"Setting `ARDUINOJSON_STRING_LENGTH_SIZE` to 2 doesn't solve the issue"},{id:"fewer-than-255",page:336,label:"No string contains more than 255 characters",summary:"No string contains more than 255 characters"}]},{content:`<p>Due to another optimization, strings in a <code>JsonDocument</code> can only contain up to 65,535 characters.</p>
<p>You can push this limit by increasing <code>ARDUINOJSON_STRING_LENGTH_SIZE</code>, like so:</p>
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Setting `ARDUINOJSON_STRING_LENGTH_SIZE` to 4 solves the issue"},{id:"string-length-size-4",page:336,label:"No",summary:"Setting `ARDUINOJSON_STRING_LENGTH_SIZE` to 4 doesn't solve the issue"},{id:"fewer-than-255",page:336,label:"No string contains more than 65,535 characters",summary:"No string contains more than 65,535 characters"}]},{content:`<p class="display-4">☹️</p>
...
`,options:[{id:"crash",page:227,label:"The program crashes",summary:"Program crashes"},{id:"empty",page:343,label:"The output is empty (e.g. <code>{}</code>, <code>[]</code>, or <code>null</code>)",summary:"Output is empty"},{id:"incomplete",page:352,label:"The output is incomplete",summary:"Output is incomplete"},{id:"garbage",page:344,label:"The output contains garbage",summary:"Output contains garbage"},{id:"null",page:352,label:"The output contains <code>null</code>",summary:"Output contains null"},{id:"float",page:238,label:"Floating-point values contain too many decimal digits",summary:"Floating-point values contain too many decimal digits"},{id:"slow",page:270,label:"It's slow",summary:"Serialization is slow"}]},{content:`<p>Please print the value of <code>JsonDocument::overflowed()</code>, like so:</p>
<pre><code class="hljs language-c++">Serial.<span class="hljs-built_in">println</span>(doc.<span class="hljs-built_in">overflowed</span>());
</code></pre>
<p>What value does it print?</p>
`,options:[{id:"overflowed-1",page:334,label:"<code>1</code> (or <code>true</code>)",summary:"`JsonDocument::overflowed()` returns `true`"},{id:"overflowed-0",page:1,label:"<code>0</code> (or <code>false</code>)",summary:"`JsonDocument::overflowed()` returns `false`"}]},{content:`<p>Which function produces the garbage?</p>
`,options:[{id:"json",page:346,label:"<code>serializeJson()</code> (or <code>serializeJsonPretty()</code>)",summary:"`serializeJson()` produces garbage"},{id:"msg",page:351,label:"<code>serializeMsgPack()</code>",summary:"`serializeMsgPack()` produces garbage"}]},{content:`<p>Do you use <a href="https://www.arduino.cc/reference/en/language/variables/data-types/stringobject/"><code>String</code></a> in your program?</p>
...
`,options:[{id:"document",page:345,label:"<code>JsonDocument</code>",summary:"Program calls `serializeJson(const JsonDocument&, ...)`"},{id:"array",page:347,label:"<code>JsonArray</code> (or <code>JsonArrayConst</code>)",summary:"Program calls `serializeJson(JsonArrayConst, ...)`"},{id:"object",page:348,label:"<code>JsonObject</code> (or <code>JsonObjectConst</code>)",summary:"Program calls `serializeJson(JsonObjectConst, ...)`"},{id:"variant",page:349,label:"<code>JsonVariant</code> (or <code>JsonVariantConst</code>)",summary:"Program calls `serializeJson(JsonVariantConst, ...)`"}]},{content:`<p><code>JsonArray</code> doesn't contain any data: it is a reference to an object stored in the <code>JsonDocument</code>. It becomes invalid as soon as the <code>JsonDocument</code> is destroyed; this could explain the garbage you see in the output.</p>
<p>For example, here is a function that creates a dangling <code>JsonArray</code>:</p>
<pre><code class="hljs language-c++"><span class="hljs-comment">// DON&#x27;T DO THAT!!!  💀</span>
<span class="hljs-function">JsonArray <span class="hljs-title">createArray</span><span class="hljs-params">()</span> </span>{
  JsonDocument doc;
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Extending the lifetime of the `JsonDocument` solves the issue"},{id:"not-destroyed",page:345,label:"No",summary:"Extending the lifetime of the `JsonDocument` doesn't solve the issue"}]},{content:`<p><code>JsonObject</code> doesn't contain any data: it is a reference to an object stored in the <code>JsonDocument</code>. It becomes invalid as soon as the <code>JsonDocument</code> is destroyed; this could explain the garbage you see in the output.</p>
<p>For example, here is a function that creates a dangling <code>JsonObject</code>:</p>
<pre><code class="hljs language-c++"><span class="hljs-comment">// DON&#x27;T DO THAT!!!  💀</span>
<span class="hljs-function">JsonObject <span class="hljs-title">createObject</span><span class="hljs-params">()</span> </span>{
  JsonDocument doc;
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Extending the lifetime of the `JsonDocument` solves the issue"},{id:"not-destroyed",page:345,label:"No",summary:"Extending the lifetime of the `JsonDocument` doesn't solve the issue"}]},{content:`<p><code>JsonVariant</code> doesn't contain any data: it is a reference to an object stored in the <code>JsonDocument</code>. It becomes invalid as soon as the <code>JsonDocument</code> is destroyed; this could explain the garbage you see in the output.</p>
<p>For example, here is a function that creates a dangling <code>JsonVariant</code>:</p>
<pre><code class="hljs language-c++"><span class="hljs-comment">// DON&#x27;T DO THAT!!!  💀</span>
<span class="hljs-function">JsonVariant <span class="hljs-title">createVariant</span><span class="hljs-params">()</span> </span>{
  JsonDocument doc;
...
<p>Did this solve your issue?</p>
`,options:[{id:"success",page:2,label:"Yes",summary:"Extending the lifetime of the `JsonDocument` solves the issue"},{id:"not-destroyed",page:345,label:"No",summary:"Extending the lifetime of the `JsonDocument` doesn't solve the issue"}]},{content:`<p><code>serializeMsgPack()</code> share a lot of code with <code>serializeJson()</code>. There is no reason you should produce garbage with one and not the other. You may have found a bug.</p>
`,tags:["needs_assistance"]},{content:`<p><a href="https://msgpack.org/">MessagePack</a> is a binary format, so if you print a document to the serial port, the result would look weird.</p>
<p>For example, the following program:</p>
<pre><code class="hljs language-c++">JsonDocument doc;
doc[<span class="hljs-string">&quot;val&quot;</span>] = <span class="hljs-number">42</span>;
...
<p>I need you to edit your program to replace <code>serializeMsgPack()</code> with <code>serializeJson()</code>.</p>
<p>Do you see the garbage in the JSON output?</p>
`,options:[{id:"json",page:346,label:"Yes, <code>serializeJson()</code> produces garbage too",summary:"`serializeJson()` produces garbage too"},{id:"deserialize-no",page:350,label:"No, <code>serializeJson()</code> doesn't produces garbage",summary:"`serializeJson()` doesn't produces garbage"}]},{content:`<p>Please print the value of <code>JsonDocument::overflowed()</code>, like so:</p>
<pre><code class="hljs language-c++">Serial.<span class="hljs-built_in">println</span>(doc.<span class="hljs-built_in">overflowed</span>());
</code></pre>
<p>What value does it print?</p>
`,options:[{id:"overflowed-1",page:334,label:"<code>1</code> (or <code>true</code>)",summary:"`JsonDocument::overflowed()` returns `true`"},{id:"overflowed-0",page:353,label:"<code>0</code> (or <code>false</code>)",summary:"`JsonDocument::overflowed()` returns `false`"}]},{content:`<p>Does one of the strings in the <code>JsonDocument</code> contains a NUL (i.e.,  ASCII code 0, or <code>\\u0000</code>)?</p>
